...
export default function EditContactForm({ initialFacilityName }: Props) {
  const router = useRouter();

  const [facility, setFacility] = useState(initialFacilityName);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data.success) {
        setStatus("送信しました。内容を確認のうえ、必要に応じてご連絡いたします。");
    ...
    <main className="min-h-screen bg-[#f7f8f5] text-gray-900">
      <section className="bg-white border-b">
        <div className="max-w-4xl mx-auto px-4 py-10">
          <h1 className="text-3xl font-bold">掲載情報の修正・削除のご相談</h1>

          <p className="mt-4 text-gray-600">
            掲載情報の修正・削除をご希望の場合は、以下のフォームよりご連絡ください。
          </p>
        </div>
      </section>
